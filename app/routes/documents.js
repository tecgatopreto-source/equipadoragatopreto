...
router.post(
  ...
  serializeImport(async (req, res) => {
    ...
    if (!["fiscal", "gerencial"].includes(type))
      return res
        .status(400)
        .json({ error: 'Tipo inválido. Use "fiscal" ou "gerencial".' });
    if (!req.file)
      return res.status(400).json({ error: "Nenhum arquivo enviado." });
    ...
    try {
      const { rows } = await pool.query(
        `SELECT id, name, snap_stock_mgmt, stock_fiscal FROM ${_s}products`,
      );
    ...
    if (extracted.length > 0) {
      ...
      const pfs = extracted.map((i) =>
        type === "fiscal" ? i.suggested.price_fiscal : null,
      );
      const sfs = extracted.map((i) =>
        type === "fiscal" ? i.suggested.stock_fiscal : null,
      );
      const spms = extracted.map((i) =>
        type === "gerencial" ? i.suggested.snap_price_mgmt : null,
      );
      const ssms = extracted.map((i) =>
        type === "gerencial" ? i.suggested.snap_stock_mgmt : null,
      );
      ...
      const qtyField =
        type === "gerencial" ? "snap_stock_mgmt" : "stock_fiscal";
      ...
      if (changedIds.length > 0) {
        ...
        `,
          ...
          console.error(
            "[upload] Erro ao registrar audit de import:",
            err.message,
          );
        }
      }
    ...
    `);
      ...
      console.error(
        "[upload] Erro ao atualizar status/histórico:",
        err.message,
      );
    }
  ...
  serializeImport(async (req, res) => {
    ...
    if (filename) {
      try {
        ...
        console.error(
          "[apply-groups] Erro ao registrar documento:",
          err.message,
        );
      }
    }
    ...
    const allIds = groups.flatMap((g) =>
      !g.skip && Array.isArray(g.productIds) ? g.productIds : [],
    );
    ...
    if (auditIds.length) {
      try {
        const prevCats = auditIds.map(
          (id) => currentMap.get(id)?.categoria ?? null,
        );
        await pool.query(
          `
        INSERT INTO ${_s}product_audit
    ...
    const totalParsed = groups.reduce(
      (s, g) => s + (g.found || 0) + (g.notFound || 0),
      0,
    );
...
router.get("/", requireAdmin, async (req, res) => {
  try {
    const pool = getDb();
    const { rows } = await pool.query(
      `SELECT * FROM ${_s}uploaded_documents ORDER BY uploaded_at DESC LIMIT 50`,
    );
...
function parseGrupos(rawText, productSet) {
  ...
  for (const line of lines) {
    ...
    if (current && isPureInt(line)) {
      if (productSet.has(line)) {
        current.productIds.push(line);
        current.found++;
      } else {
        current.notFound++;
      }
  ...
  allLines.forEach((l, i) => {
    if (
      ...
      for (
        ...
        j <= Math.min(allLines.length - 1, i + 9);
        j++
      ) {
        headerIdx.add(j);
      }
  ...
  for (let i = 0; i < dataLines.length; i++) {
    ...
    if (!isNaN(stock) && !isNaN(price)) {
      ...
      results.push({
        ...
        is_disabled,
        current:
          existing && type === "fiscal"
            ? {
                price_fiscal: existing.price_fiscal,
                stock_fiscal: existing.stock_fiscal,
              }
            : existing
              ? {
                  snap_price_mgmt: existing.snap_price_mgmt,
                  snap_stock_mgmt: existing.snap_stock_mgmt,
                }
              : {},
        suggested:
          type === "fiscal"
